...
@socketio.on("join")
def on_join(data):
    room = f"room_{min(data['user_id'], data['other_id'])}_{max(data['user_id'], data['other_id'])}"
    join_room(room)
...
def on_message(data):
    ...
    conn.execute("""
        INSERT INTO message (id_expediteur, id_destinataire, contenu, date_envoi)
        VALUES (?, ?, ?, ?)
    """, (data["user_id"], data["other_id"], data["contenu"], today_iso()))
    conn.commit()

    user = conn.execute(
        "SELECT username, role FROM client WHERE id_client=?", (data["user_id"],)
    ).fetchone()
    conn.close()
